...
export const TRANSITIONS = Object.freeze({
  READY: new Set(["RUNNING", "INTERRUPTED", "STALE_BASE", "ENVIRONMENT_DRIFT", "BUDGET_REVIEW", "PERMISSION_BLOCKED", "AWAITING_CODEX_REVIEW"]),
  RUNNING: new Set(["VERIFYING", "INTERRUPTED", "BUDGET_EXHAUSTED", "PERMISSION_BLOCKED", "AWAITING_CODEX_REVIEW"]),
  VERIFYING: new Set(["CORRECTING", "AWAITING_USER", "VERIFICATION_FAILED", "PERMISSION_BLOCKED", "INTERRUPTED", "AWAITING_CODEX_REVIEW"]),
  CORRECTING: new Set(["VERIFYING", "INTERRUPTED", "BUDGET_EXHAUSTED", "PERMISSION_BLOCKED", "AWAITING_CODEX_REVIEW"]),
  AWAITING_USER: new Set(["AWAITING_CODEX_REVIEW", "RETURNED", "USER_OWNED", "CANCELLED"]),
  INTERRUPTED: new Set(["RUNNING", "STALE_BASE", "ENVIRONMENT_DRIFT", "BUDGET_REVIEW", "BUDGET_EXHAUSTED", "PERMISSION_BLOCKED", "AWAITING_CODEX_REVIEW", "USER_OWNED", "CANCELLED"]),
  VERIFICATION_FAILED: new Set(["AWAITING_CODEX_REVIEW", "RETURNED", "USER_OWNED", "CANCELLED"]),
  BUDGET_EXHAUSTED: new Set(["AWAITING_CODEX_REVIEW", "USER_OWNED", "CANCELLED"]),
  BUDGET_REVIEW: new Set(["AWAITING_CODEX_REVIEW", "CANCELLED"]),
  STALE_BASE: new Set(["AWAITING_CODEX_REVIEW", "CANCELLED"]),
  ENVIRONMENT_DRIFT: new Set(["AWAITING_CODEX_REVIEW", "CANCELLED"]),
  PERMISSION_BLOCKED: new Set(["AWAITING_CODEX_REVIEW", "USER_OWNED", "CANCELLED"]),
...
const CODEX_ONLY_PATHS = Object.freeze([
  ...
  ".opencode-subagent/contract.example.json",
  ".opencode-subagent/drafts/**",
  ".opencode-subagent/tasks/**",
...
export function validateActiveContract(contract, taskId, policy = {}) {
  ...
  if (contract.approval?.status !== "APPROVED" || typeof contract.approval?.approvedBy !== "string"
      || !contract.approval.approvedBy.trim() || contract.approval.approvedBy.toLowerCase() === "opencode"
  ...
  const reservedPaths = [...CODEX_ONLY_PATHS, ...(policy.protectedPaths ?? [])];
  const protectedPaths = contract.scope.allowedPaths.filter((allowed) =>
    reservedPaths.some((reserved) => rulesOverlap(allowed, reserved))
  );
  ...
  if (typeof contract.resultPath !== "string" || !contract.scope.allowedPaths.some((rule) => matchesRule(contract.resultPath, rule))) {
    throw new Error("Task resultPath must be included in scope.allowedPaths");
  }
  for (const command of contract.verificationCommands) validateCommand(command, policy);
  return contract;
}
...
export function validateCommand(command, policy = {}) {
  ...
  const allowedExecutables = new Set(policy.verificationExecutables ?? [
    "node", "python", "uv", "ruff", "pnpm", "npm", "npx", "cargo", "go", "dotnet"
  ]);
...
export function runtimePaths(root, taskId) {
  const base = path.join(root, ".opencode-subagent", "runtime");
  return {
    base,
...
export function createInitialState(taskId, contract) {
  return {
    ...
    status: "READY",
    owner: contract.approval.approvedBy,
    approvalMode: contract.approval.mode ?? "MANUAL_GATES",
    grants: contract.approval.grants ?? [],
    pushAuthorized: contract.approval.pushAuthorized === true,
